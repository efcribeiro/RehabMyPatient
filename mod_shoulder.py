...
def main():
    ...
    # Defina a resolução desejada (1920x1080)
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        while cap.isOpened():
            ...
            try:
                ...
                angle_d = calculate_angle(elbow_d, shoulder_d, hip_d)

                # Calcula angulo braco esquerdo
                angle_e = calculate_angle(elbow_e, shoulder_e, hip_e)

                # Mostra angulos na tela - Esquerda
                cv2.putText(
                    image,
                    f"ESQ: {str(angle_e)}",
                    tuple(np.multiply(shoulder_e, [900, 600]).astype(int)),
                ...
                cv2.putText(
                    image,
                    f"DIR: {str(angle_d)}",
                    tuple(np.multiply(shoulder_d, [900, 600]).astype(int)),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.5,
                    (255, 255, 255),
                    2,
            ...
            cv2.imshow("Abducao e aducao de ombro", image)
